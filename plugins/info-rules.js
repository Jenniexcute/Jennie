...
let rules = `➯ *Do not spam to Bᴏᴛ*
 ⟬⟭
➯ *your number will ban!*
⟬Never you can not use bot again!⟭
➯ *Cᴀʟʟ AᴜᴛᴏBʟᴏᴄᴋ*
⟬if you call bot,think about it again⟭
➯ *bot is not provide hers script*
⟬then do not bothered for bot⟭

⟬this is minimum rules of bot!⟭
❏┳━━◩
┍┛
┆⟥⟤ ➠ (${global.bottime})
└─┈⟅`
...
conn.send3ButtonImg(m.chat, `${pickRandom(flaaa2)}` + `${ucapan()} ` + `${name}`, nth, rules, 'Menu', '.menu', 'Owner', '.owner', 'Credit', '.credit', m, { contextInfo: { externalAdReply: { showAdAttribution: true,
    ...
    mediaType: 2, 
    description: sgc,
    title: "rent check here,but this bot is always for free!!!",
    body: wm,
